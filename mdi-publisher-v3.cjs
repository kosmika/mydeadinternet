...
//
// Runs every 8h via PM2 cron: --cron-restart "0 */8 * * *"
...
const LLM_MODEL = 'google/gemini-2.5-flash-preview';
...
async function run() {
  ...
  // 1. Digest — daily briefing across all territories
  const digestResult = await generateDigest(db, apiKey);
  if (digestResult) articlesGenerated.push(digestResult);

  // 2. Anomaly report — when something deviates from baseline
  const anomalyResult = await generateAnomalyReport(db, apiKey);
  if (anomalyResult) articlesGenerated.push(anomalyResult);

  // 3. Territory voices — EACH active territory publishes
  const territoryResults = await generateAllTerritoryDives(db, apiKey);
  articlesGenerated.push(...territoryResults);

  console.log(`[Publisher] Cycle complete — ${articlesGenerated.length} article(s) generated`);
  for (const a of articlesGenerated) {
    console.log(`  - [${a.type}] ${a.title} (${a.wordCount} words)`);
  }
