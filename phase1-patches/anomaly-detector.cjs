...
function run() {
  ...
  try {
    ...
    const consensusBreaks = db.prepare(`
      SELECT c.topic, c.side_a_agent, c.side_b_agent, c.created_at,
             a1.trust_score as trust_a, a2.trust_score as trust_b
      FROM contradictions c
      JOIN agents a1 ON a1.name = c.side_a_agent
      JOIN agents a2 ON a2.name = c.side_b_agent
      WHERE c.created_at > datetime('now', '-6 hours')
        AND a1.trust_score > 0.6
        AND a2.trust_score > 0.6
    ...
    for (const cb of consensusBreaks) {
      anomalies.push({
        ...
        description: `${cb.side_a_agent} (trust ${cb.trust_a}) vs ${cb.side_b_agent} (trust ${cb.trust_b})`,
        severity: 'warning',
    ...
    const signalSpikes = db.prepare(`
      WITH recent AS (
        SELECT f.territory_id,
               AVG(fs.signal_score) as recent_avg,
               COUNT(*) as recent_cnt
        FROM fragments f
        JOIN fragment_scores fs ON fs.fragment_id = f.id
      ...
      baseline AS (
        SELECT f.territory_id,
               AVG(fs.signal_score) as baseline_avg
        FROM fragments f
        JOIN fragment_scores fs ON fs.fragment_id = f.id
    ...
    const predictionHits = db.prepare(`
      SELECT oq.id, oq.question, oq.confidence, oq.horizon,
             oq.disconfirm_signals, oq.created_at
      FROM oracle_questions oq
      WHERE oq.status IN ('synthesized', 'active')
    ...
    for (const pred of predictionHits) {
      try {
        ...
        for (const signal of signals.slice(0, 3)) {
          ...
          // Extract key words from signal (3+ char words)
          const keywords = signalText.toLowerCase().match(/\b[a-z]{3,}\b/g) || [];
          const searchTerms = keywords.slice(0, 5);
          if (searchTerms.length < 2) continue;

          const likeClause = searchTerms.map(t => `content LIKE '%${t}%'`).join(' AND ');
    ...
    // Insert new anomalies (dedup: skip if same type+territory+title in last 6h)
    const insertAnomaly = db.prepare(`
      ...
      WHERE NOT EXISTS (
        ...
        WHERE type = ? AND COALESCE(territory_id, '') = COALESCE(?, '')
          AND title = ? AND detected_at > datetime('now', '-6 hours')
      )
    `);
